...
char * clientComm(int clntSockfd,int * senderBuffSize_addr, int * optlen_addr){
    ...
    //By inserting this canary value onto the stack, I am preventing anyone from using the buffer input to overwrite the return address.
    //This works because the attacker would have to craft an input that contains this canary value. However, the strcpy function will not
    //be able to get past the NULL byte. Therefore, an overwrite of the return address using this buffer is impossible.
    int canary = 0x000aff0d;
    ...
    {
        //This code will excecute if a buffer overflow attack is attempted
