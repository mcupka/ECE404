...
def encrypt(input_fname: str, output_fname: str):
    ...
    # remove the header info for the ppm file and place it at the top of the output
    # this loop prints characters verbatim to the output until 3 newlines have been reached
    newline_count = 0
    while newline_count < 3:
        one_char = bv.read_bits_from_file(8)
        one_char.write_to_file(output_file)
        if one_char.get_bitvector_in_ascii() == '\n':
            newline_count += 1

    # Now encrypt the remaining data just like the text file
    ...
    while bv.more_to_read:
        ...
        blocks_written += 1
        print(f'Block: {blocks_written} written to file')


if __name__ == "__main__":
    encrypt("image.ppm", "image_enc.ppm")
